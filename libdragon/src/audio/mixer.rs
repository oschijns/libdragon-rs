...
pub fn init(num_channels: i32) {
    unsafe {
        libdragon_sys::mixer_init(num_channels);
    }
}
...
pub fn set_vol(vol: f32) {
    unsafe {
        libdragon_sys::mixer_set_vol(vol);
    }
}
...
pub fn ch_set_vol(ch: i32, lvol: f32, rvol: f32) {
    unsafe {
        libdragon_sys::mixer_ch_set_vol(ch, lvol, rvol);
    }
}
...
pub fn ch_set_vol_pan(ch: i32, lvol: f32, rvol: f32) {
    unsafe {
        libdragon_sys::mixer_ch_set_vol_pan(ch, lvol, rvol);
    }
}
...
pub fn ch_set_vol_dolby(ch: i32, fl: f32, fr: f32, c: f32, sl: f32, sr: f32) {
    unsafe {
        libdragon_sys::mixer_ch_set_vol_dolby(ch, fl, fr, c, sl, sr);
    }
}
...
pub fn ch_set_freq(ch: i32, frequency: f32) {
    unsafe {
        libdragon_sys::mixer_ch_set_freq(ch, frequency);
    }
}
...
pub fn ch_set_pos(ch: i32, pos: f32) {
    unsafe {
        libdragon_sys::mixer_ch_set_pos(ch, pos);
    }
}
...
pub fn ch_stop(ch: i32) {
    unsafe {
        libdragon_sys::mixer_ch_stop(ch);
    }
}
...
pub fn ch_set_limits(ch: i32, max_bits: i32, max_frequency: f32, max_buf_sz: usize) {
    unsafe {
        libdragon_sys::mixer_ch_set_limits(ch, max_bits, max_frequency, max_buf_sz as i32);
    }
}
...
pub fn throttle(num_samples: f32) {
    unsafe {
        libdragon_sys::mixer_throttle(num_samples);
    }
}
...
pub fn unthrottle() {
    unsafe {
        libdragon_sys::mixer_unthrottle();
    }
}
...
pub fn add_event(delay: i64, cb: MixerEventCallback) -> MixerEvent {
    let cb = Box::new(MixerEventInternal {
        user_callback: cb,
        rc:            AtomicU32::new(2),
    });
    let ctx = unsafe {
...
impl MixerEvent {
    ...
    pub fn remove(&mut self) {
        if let Some(ctx) = self.0 {
            ...
            unsafe {
                libdragon_sys::mixer_remove_event(
                    Some(event_callback),
                    ctx as *mut ::core::ffi::c_void,
                );
            }
...
pub type WaveformReadCallback =
    Box<dyn FnMut(&mut samplebuffer::SampleBuffer, usize, usize, bool) + 'static + Sync + Send>;
...
impl Waveform {
    pub(crate) fn from_ptr(ptr: *mut libdragon_sys::waveform_s) -> Self { Self { ptr } }

    /// Start playing the waveform on the specified channel.
    ///
    /// See [`mixer_ch_play`](libdragon_sys::mixer_ch_play) for details.
    pub fn play(&self, ch: i32) {
        unsafe {
            libdragon_sys::mixer_ch_play(ch, self.ptr);
        }
    }
    ...
    pub fn frequency(&self) -> f32 { unsafe { (*self.ptr).frequency } }
    ...
    pub fn loop_len(&self) -> usize { unsafe { (*self.ptr).loop_len as usize } }
}
