...
impl Eepfs {
    ...
    pub fn add<T: AsRef<dfs::Path>>(&mut self, path: T, size: usize) -> &mut Self {
        ...
        self.e
            .push(libdragon_sys::eepfs_entry_t { path: ptr, size });
    ...
    /// let fs = Eepfs::new()
    ///     .add(
    ///         dfs::PathBuf::from("/high_scores.dat"),
    ///         ::core::mem::size_of::<HighScores>(),
    ///     )
    ///     .add(
    ///         dfs::PathBuf::from("/settings.dat"),
    ///         ::core::mem::size_of::<Settings>(),
    ///     )
    ///     .add(
    ///         dfs::PathBuf::from("/player.sav"),
    ///         ::core::mem::size_of::<SaveData>(),
    ///     )
    ///     .init()
    ///     .expect("Could not initialize EEPROMFS");
    /// ```
    ///
    ...
    pub fn read<T: AsRef<dfs::Path>>(&self, path: T, size: usize) -> Result<Vec<u8>> {
        ...
        let v =
            unsafe { libdragon_sys::eepfs_read(cpath.as_ptr(), res.as_mut_ptr() as *mut _, size) };
        if v != EEPFS_ESUCCESS {
            return Err(LibDragonError::EepfsError { error: v.into() });
        }
        Ok(res)
    ...
    /// See [`eepfs_erase`](libdragon_sys::eepfs_erase) for details.
    #[inline]
    pub fn verify_signature(&self) -> bool { unsafe { libdragon_sys::eepfs_verify_signature() } }
