...
pub fn ticks() -> u64 { unsafe { libdragon_sys::timer_ticks() as u64 } }

/// Calculate microseconds based on timer ticks
pub fn micros(ticks: u64) -> u64 { (ticks * 1000000) / (ticks::per_second() as u64) }
...
#[inline(always)]
pub fn make_ticks(us: u64) -> i32 { ((us * (ticks::per_second() as u64)) / 1000000) as i32 }
...
impl Timer {
    ...
    pub fn new(
        ticks: i32,
        mode: Mode,
        callback: Box<dyn Fn(i32) + 'static + Sync + Send>,
    ) -> Timer {
        ...
        let (timer_link, ctx) = unsafe {
            ...
            (
                libdragon_sys::new_timer_context(
                    ticks,
                    mode.into(),
                    Some(timer_callback),
    ...
    pub fn start(
        ...
        let ctx = unsafe {
            let ctx: *mut TimerCallback = Box::leak(cb);
            libdragon_sys::start_timer_context(
                self.ptr,
                ticks,
                mode.into(),
                Some(timer_callback),
