...
pub fn make_color(r: u8, g: u8, b: u8, a: u8) -> u32 {
    unsafe { libdragon_sys::graphics_make_color(r as i32, g as i32, b as i32, a as i32) }
}
...
pub fn convert_color(color: Color) -> u32 {
    unsafe { graphics_convert_color_r(&color.c as *const libdragon_sys::color_t) }
}
...
pub fn rgba32(r: u8, g: u8, b: u8, a: u8) -> Color {
    Color {
        c: libdragon_sys::color_t { r, g, b, a },
    }
}
...
impl<'a> Graphics<'a> {
    ...
    #[inline]
    pub fn surface(&self) -> Option<&Surface<'a>> { self.surface.as_ref() }

    /// Return a mutable reference to the [Surface] this [Graphics] context owns.
    #[inline]
    pub fn surface_mut(&mut self) -> Option<&mut Surface<'a>> { self.surface.as_mut() }
    ...
    pub fn draw_pixel_trans(&mut self, x: i32, y: i32, color: u32) {
        unsafe {
            libdragon_sys::graphics_draw_pixel_trans(
                self.surface.as_mut().unwrap().ptr,
                x,
                y,
                color,
            );
    ...
    pub fn draw_line(&mut self, x0: i32, y0: i32, x1: i32, y1: i32, color: u32) {
        unsafe {
            libdragon_sys::graphics_draw_line(
                self.surface.as_mut().unwrap().ptr,
                x0,
                y0,
                x1,
    ...
    pub fn draw_line_trans(&mut self, x0: i32, y0: i32, x1: i32, y1: i32, color: u32) {
        unsafe {
            libdragon_sys::graphics_draw_line_trans(
                self.surface.as_mut().unwrap().ptr,
                x0,
                y0,
                x1,
    ...
    pub fn draw_box(&mut self, x0: i32, y0: i32, x1: i32, y1: i32, color: u32) {
        unsafe {
            libdragon_sys::graphics_draw_box(
                self.surface.as_mut().unwrap().ptr,
                x0,
                y0,
                x1,
    ...
    pub fn draw_box_trans(&mut self, x0: i32, y0: i32, x1: i32, y1: i32, color: u32) {
        unsafe {
            libdragon_sys::graphics_draw_box_trans(
                self.surface.as_mut().unwrap().ptr,
                x0,
                y0,
                x1,
    ...
    pub fn draw_character(&mut self, x: i32, y: i32, c: char) -> Result<()> {
        unsafe {
            libdragon_sys::graphics_draw_character(
                self.surface.as_mut().unwrap().ptr,
                x,
                y,
                c.as_ascii()
                    .ok_or(LibDragonError::Utf8Error { error: None })?
    ...
    pub fn draw_text(&mut self, x: i32, y: i32, msg: &str) {
        ...
        unsafe {
            libdragon_sys::graphics_draw_text(
                self.surface.as_mut().unwrap().ptr,
                x,
                y,
                cmsg.as_ptr(),
    ...
    pub fn draw_sprite(&mut self, x: i32, y: i32, sprite: &Sprite) {
        unsafe {
            libdragon_sys::graphics_draw_sprite(
                self.surface.as_mut().unwrap().ptr,
                x,
                y,
                sprite.as_const_sprite_s() as *mut libdragon_sys::sprite_s,
            );
        }
    }
    ...
    pub fn draw_sprite_stride(&mut self, x: i32, y: i32, sprite: &Sprite, offset: i32) {
        unsafe {
            libdragon_sys::graphics_draw_sprite_stride(
                self.surface.as_mut().unwrap().ptr,
                x,
                y,
                sprite.as_const_sprite_s() as *mut libdragon_sys::sprite_s,
    ...
    pub fn draw_sprite_trans(&mut self, x: i32, y: i32, sprite: &Sprite) {
        unsafe {
            libdragon_sys::graphics_draw_sprite_trans(
                self.surface.as_mut().unwrap().ptr,
                x,
                y,
                sprite.as_const_sprite_s() as *mut libdragon_sys::sprite_s,
    ...
    pub fn draw_sprite_trans_stride(&mut self, x: i32, y: i32, sprite: &Sprite, offset: i32) {
        unsafe {
            libdragon_sys::graphics_draw_sprite_trans_stride(
                self.surface.as_mut().unwrap().ptr,
                x,
                y,
                sprite.as_const_sprite_s() as *mut libdragon_sys::sprite_s,
...
pub fn set_font_sprite<'b>(sprite: Sprite<'b>) -> core::pin::Pin<Box<Sprite<'b>>> {
    ...
    unsafe {
        libdragon_sys::graphics_set_font_sprite(
            pinned.as_const_sprite_s() as *mut libdragon_sys::sprite_s
        );
    }
    pinned
}
