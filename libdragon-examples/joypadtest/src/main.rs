...
extern "C" fn main() -> ! {
    ...
    let fmt_inputs = |inputs: &joypad::Inputs| {
        let mut s = format!(
            "Stick: {:+04},{:+04} C-Stick: {:+04},{:+04} L-Trig:{:03} R-Trig:{:03}\n",
            inputs.stick_x,
            inputs.stick_y,
            inputs.cstick_x,
            inputs.cstick_y,
        ...
        s.push_str(&format!(
            "D-U:{} D-D:{} D-L:{} D-R:{} C-U:{} C-D:{} C-L:{} C-R:{}\n",
            inputs.btn.d_up as u8,
            inputs.btn.d_down as u8,
            inputs.btn.d_left as u8,
            inputs.btn.d_right as u8,
            inputs.btn.c_up as u8,
            inputs.btn.c_down as u8,
            inputs.btn.c_left as u8,
            inputs.btn.c_right as u8
        ));
        s.push_str(&format!(
            "A:{} B:{} X:{} Y:{} L:{} R:{} Z:{} Start:{}\n",
            inputs.btn.a as u8,
