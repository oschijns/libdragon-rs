...
extern "C" fn main() -> ! {
    ...
    let _timer = Timer::new(
        timer::make_ticks(1000000 / 30),
        timer::Mode::Continuous,
        Box::new(move |_| {
            animcounter_clone.store(
                animcounter_clone.load(Ordering::SeqCst) + 1,
                Ordering::SeqCst,
            );
        }),
    );
    ...
    loop {
        ...
        match mode {
            0 => {
                ...
                g.draw_sprite_stride(
                    20,
                    100,
                    &earthbound,
                    if ((a / 15) & 1) != 0 { 1 } else { 0 },
                );
